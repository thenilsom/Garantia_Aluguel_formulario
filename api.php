...
function salvar($request, $response){
	...
	$tipo_DOC_inquilino = trim(json_encode($cadastro->pretendente->tipoDoc), '"');
	$DOC_inquilino = trim(json_encode($cadastro->pretendente->numDoc), '"');
...
//*********************************************************************************	$tem_renda_arcar_loc_inquilino = trim(json_encode($cadastro->pretendente->possuiRendaArcarLocacao), '"');
	...
	$nome_imobiliaria = trim(json_encode($cadastro->residencia->nomeImobiliaria), '"');
	$telefone_imobiliaria = trim(json_encode($cadastro->residencia->telefoneImobiliaria), '"');
...
//	$outros_imov_alugados_inquilino = trim(json_encode($cadastro->residencia->outrosImoveisAlugados), '"');
	...
	if(json_encode($cadastro->residencia->anterior->complemento) <> "null"){$complemento_anterior_inquilino = trim(json_encode($cadastro->residencia->anterior->complemento), '"');}else{$complemento_anterior_inquilino="";}
	...
	$fone_com_2_inquilino = trim(json_encode($cadastro->profissional->telefone), '"');
...
//	$profComplemento = trim(json_encode($cadastro->profissional->complemento), '"');
	...
	$num_solidarios = trim(json_encode($cadastro->pessoal->numSolidarios), '"');
	$solidario1 = trim(json_encode($cadastro->pessoal->solidario1->nome), '"');
	$solidario1_cpf = trim(json_encode($cadastro->pessoal->solidario1->cpf), '"');
	$solidario1_fone = trim(json_encode($cadastro->pessoal->solidario1->telefone), '"');
	$solidario1_sexo = trim(json_encode($cadastro->pessoal->solidario1->sexo), '"');
	$solidario1_rg = trim(json_encode($cadastro->pessoal->solidario1->numDoc), '"');
...
//	$solidario2_data_exp_rg = trim(json_encode($cadastro->pessoal->ocupante1->dataEmissao), '"');
	$solidario2 = trim(json_encode($cadastro->pessoal->solidario2->nome), '"');
	$solidario2_cpf = trim(json_encode($cadastro->pessoal->solidario2->cpf), '"');
	$solidario2_fone = trim(json_encode($cadastro->pessoal->solidario2->telefone), '"');
	$solidario2_sexo = trim(json_encode($cadastro->pessoal->solidario2->sexo), '"');
	$solidario2_rg = trim(json_encode($cadastro->pessoal->solidario2->numDoc), '"');
...
//	$solidario2_data_exp_rg = trim(json_encode($cadastro->pessoal->ocupante2->dataEmissao), '"');
	$solidario3 = trim(json_encode($cadastro->pessoal->solidario3->nome), '"');
	$solidario3_cpf = trim(json_encode($cadastro->pessoal->solidario3->cpf), '"');
	$solidario3_fone = trim(json_encode($cadastro->pessoal->solidario3->telefone), '"');
	$solidario3_sexo = trim(json_encode($cadastro->pessoal->solidario3->sexo), '"');
	$solidario3_rg = trim(json_encode($cadastro->pessoal->solidario3->numDoc), '"');
...
//	$solidario3_data_exp_rg = trim(json_encode($cadastro->pessoal->ocupante3->dataEmissao), '"');
    ...
    $sql = "insert into fianca values ('', '$data_cob', '$hora_cob', '$seguradora', '', '$usuario_upload', '', '', '', '', '', '', '', '',
								   ...
								   '$tempo_resid_inquilino', '$tipo_resid_inquilino', '$resid_emnomede_inquilino', '$arca_com_aluguel_inquilino', '$aluguel_atual_inquilino', '$outros_imov_alugados_inquilino', '$cep_anterior_inquilino', '$uf_anterior_inquilino', '$cidade_anterior_inquilino', '$endereco_anterior_inquilino', '$bairro_anterior_inquilino', '$complemento_anterior_inquilino', '$num_anterior_inquilino',
